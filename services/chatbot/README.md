...
### ⚡ Quick start web UI (path: `services/chatbot`)
...
```powershell
Set-Location "D:\pdf-processing-ui\services\chatbot"
& "D:\pdf-processing-ui\conversion_env\Scripts\python.exe" -m uvicorn api.main:app --host 127.0.0.1 --port 8010
```

### Troubleshooting (Windows)
- If you see `ModuleNotFoundError: No module named 'api'`, you are not running from `services/chatbot` (cwd must be that folder for `api` package).
