        ...
        if local == 'p':
            ...
            if grid:
                ...
                for r in grid:
                    while len(r) < width:
                        r.append("")
                # Word vertical merge: only the first row of a group has text;
                # continuation rows often have empty <w:tc>. Carry value downward per column.
...
def parse_docx_fast(docx_path: str, doc_id: str = None, file_name: str = None) -> DocumentStructure:
    """
    Fast DOCX parser (OCR_LLM-like behavior):
    - Extract plain text quickly
    - Build a minimal element list for fast indexing/search
    This trades rich structure fidelity for speed.
    """
    docx_path = Path(docx_path).resolve()
    if not docx_path.exists():
        raise FileNotFoundError(f"DOCX file not found: {docx_path}")
    ...
    element = DocumentElement(
        ...
        type="paragraph",
        content=full_text,
        metadata={"mode": "fast_docx"},
    )
    element.context = f">>> [{element.id}] {element.content}"
    ...
    try:
        ...
        if text_ratio > 0.8:
            ...
            process_pdf_to_docx(
                ...
                enable_ocr=True,
                load_8bit=False,  # User requested to load normally without 8-bit quantization
            )
...
def ingest_file(file_path: str, upload_dir: str = None) -> DocumentStructure:
    ...
    if suffix == '.pdf':
        ...
        # Default to fast mode for DOCX to match OCR_LLM-like ingest speed.
        # Set CHATBOT_DOCX_PARSE_MODE=full if you need rich element granularity.
        parse_mode = os.getenv("CHATBOT_DOCX_PARSE_MODE", "fast").lower()
        if parse_mode == "full":
            return parse_docx(docx_path, doc_id=doc_id, file_name=file_path.name)
        return parse_docx_fast(docx_path, doc_id=doc_id, file_name=file_path.name)
