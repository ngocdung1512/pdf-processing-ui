...
def process_pdf_to_docx(
    ...
    # Repo root: YOLO .pt in models/ (preferred) or repo root; Qwen folder at repo root
    _repo_root = find_monorepo_root(Path(__file__))
    model_path = resolve_yolo_weights(_repo_root, model_path)
    if ocr_model_path is None:
        ocr_model_path = str(_repo_root / "Qwen2.5-VL-3B")
    else:
